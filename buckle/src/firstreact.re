...
external createElement : 'thing => domConfig? => 'contents => reactElement = "React.createElement" [@@bs.val];
...
let counter = createClass(class_config
  ...
  render::(fun () => {
    ...
    let x : reactThis = [%bs.raw{|this|}];
    createElement "button" (makeMoreDomConfig onClick::(fun () => {
      (setState x (makeState (count + 1)))
...
let renderExample = fun () => {
  ...
  render (createElement counter (makeDomConfig ()) "hello") (getElementById "container");
};
